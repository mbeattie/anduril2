...
#ifdef USE_JIMMY_MODE
inline void jimmy_iter() {
    // one iteration of main loop()
    int16_t brightness;
    uint16_t pulse_time;

    pulse_time = 128; // one unit
    brightness = 32;
    if (brightness > MAX_LEVEL) brightness = MAX_LEVEL;

    // J
    set_level(brightness);
    nice_delay_ms(pulse_time);
    set_level(0);
    nice_delay_ms(pulse_time);

    set_level(brightness);
    nice_delay_ms(3 * pulse_time);
    set_level(0);
    nice_delay_ms(pulse_time);
    ...
    set_level(0);
    nice_delay_ms(2048);  // no return check necessary on final delay
}
#endif
